...
#Si tu as bien compris, quand tu lances le script, les réponses aux questions 
#s'affichent sur ton écran de Terminal via des puts. N'hésite pas à ajouter un peu de 
#pep’s à ton programme en écrivant des phrases, en ajoutant des blagues, voir en faisant un menu 
#(pour les plus chauds) où l'utilisateur peut choisir, dans une liste, la question à laquelle 
#le programme va répondre.
...
# création de l'array journalists
...
puts " Salut, quelle fonction veux tu lancer?"
puts "1 - déterminer le nb de journalistes"
puts "2 - déterminer cb d'handle ont un numéro"
puts "3 - le nb d'handle qui contiennent 'aude'"
...
puts "9 - qu'elle est la position dans l'array de la personne @epenser ?"
...
$choix= gets.chomp.to_i
	...
	if $choix== 1
		...
		puts "Choix inconnu, essaye encore "
		perform
	end
end
...
def number
	puts "Cet array comporte #{$journalists.length} journalistes"
...
def compte
	puts "il y a #{$journalists.count {|x| x.match /[0-9]/ }} handle avec un numéro"
end

#Combien d'handle contiennent les 4 lettres du prénom "Aude" à la suite (sans prendre en compte les majuscules) ?

def aude
	puts "il y a #{$journalists.count {|e| e=~ /aude/i }} handle avec aude"
end

#Combien commencent par une majuscule (première lettre juste après le @) ?
def commenceMaj
	puts "il y a #{$journalists.count {|e| e=~ /@[A-Z]/ }} handle qui commencent par une majuscule"
end

#Combien contiennent une majuscule ?
def contentMaj
	puts "il y a #{$journalists.count {|e| e=~ /[A-Z]/ }} handle qui contiennent une majuscule"
end

#Combien y a-t-il de underscore _ dans tous les pseudos confondus ?
def contentUnder
	puts "il y a #{$journalists.count {|x| x.match /[_]/ }} handle qui contiennent un underscore"
end
...
def trieLong
	puts "Voici les 50 handles les plus courts de cet array:"
	puts  $journalists.sort_by {|x| x.length}[0..49]
end
...
def position
	puts  "la position de @epense est #{$journalists.index("@epenser")} (sachant que la position du 1er élement est 0)"
end
